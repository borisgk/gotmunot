...
func createZipArchive(taskID, username string, filenames []string, archiveType string) {
	...
	for i, filename := range filenames {
		...
		if archiveType == "originals" {
			sourcePath = filepath.Join(config.AppConfig.PhotoUploadDir, photo.UploadedBy, "originals", photo.Filepath)
		} else {
			sourcePath = filepath.Join(config.AppConfig.PhotoUploadDir, photo.UploadedBy, "previews", photo.Filepath)
		}
